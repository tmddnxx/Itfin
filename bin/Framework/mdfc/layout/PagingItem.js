...
*/
class PagingItem extends AView
{
    constructor()
    {
        super()
...
window.PagingItem = PagingItem

PagingItem.prototype.init = function(context, evtListener)
{
...
//paging컴포넌트 가로길이 재정의
PagingItem.prototype.updateWidth = function()
{
	/*
...
//paging컴포넌트 가운데 처리함수
PagingItem.prototype.pagingCenter = function()
{
	/*
	if(this.isCenter)
	{
...
//페이지버튼 객체 생성
PagingItem.prototype.createPageButton = function(stl, txt, data)	//스타일헤드, 텍스트, 데이터(key/value)
{
	var defaultStyle, 
...
//페이지뷰 세팅
PagingItem.prototype.setPageView = function()
{
	//페이징 초기화(remove)
	this.removeChildren();
...
//모든버튼 스타일 초기화
/*PagingItem.prototype.onInitStyleButton = function()
{
...
//페이지 버튼 클릭했을 경우.
PagingItem.prototype.onPageBtnClick = function(acomp, info)
{
...
// 컴포넌트 내부에 드랍 가능여부 리턴
PagingItem.prototype.getDroppable = function()
{
	return false;
};
