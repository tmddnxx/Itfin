...
afc.import('Framework/mdfc/layout/ACalendarItem.js');
afc.import('Framework/mdfc/layout/ACalendarMonthItem.js');

//반드시 선행 로드되어야 한다면 마지막에 하나만 await 해야 함.
//선행 로드되어야 하는 js 가 여러개라면 마지막 js 에만 await 를 하고 다른 것들은 그 이전에 import 를 해야함.
await afc.import('Framework/mdfc/layout/ACalendarPickerItem.js');


class ACalendarPicker extends AView
{
    constructor()
    {
        super()
...
ACalendarPicker.prototype.createElement = function(context)
{
	...
	if(this.$ele.children().length==0) 
	{
		...
		afc.loadHtmlSync(item, 'Framework/mdfc/layout/ACalendarPickerItem.html');
