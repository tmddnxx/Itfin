...
AGrid.prototype.columnResizable = function(callback)
{
    ...
    //저장하지 않고 동적으로 구해온다.
    let $headCols = this.$hRowTmpl.eq(0).children('td'),
        endIdx = $headCols.length/hRowCount - 1
	
	$headCols.each(function(i)
	{
    ...
    if(!this.option.isWidthChangable) this.setColumnWidth(colLen-1, '', true);
};
...
AGrid.prototype._resizeProcess = function($sizeBar)
{
    ...
    { 
        ...
        stop: function( event, ui ) 
        {
            ...
            thisObj._colculResizeWidth($sizeBar._inx, ui.position.left);
...
AGrid.prototype._colculResizeWidth = function(inx, newLeft)
{
	...
	if((!val || val.indexOf('%')>-1) && !this.option.isWidthChangable)
	{
		...
		this.setColumnWidth(inx, moveWidth+'%', true);
	}
	else
	{
