...
afc.loadScript('Framework/afc/library/DnDManager.js');
...
ATree.prototype.setItem = function(item, itemInfo)//itemInfo -> name, data, icon, action
{
	for(var p in itemInfo)
	{
		...
		else if(p=='icon') this.changeIcon(item, itemInfo.icon); //(item).children('span').css('background-position', (-16 * itemInfo.icon) + 'px 0px');
...
ATree.prototype.insertItemObj = function(itemInfo, isExpand, mergeHistory)//itemInfo -> pItem(parent item), pos, name, data, icon, action, isGroup
{
	...
	*/
    ...
    if(this.iconMap && itemInfo.icon!=undefined) 
    {
		var $icon = $('<span></span>');
		item.$icon = $icon;
		this.changeIcon(item, itemInfo.icon);
        item.append($icon);
    }
    ...
    if(this.option.isDraggable)
    {
		...
		if(itemInfo.isDrop)
		{
			this._itemDropManage($(itemDom).children('label')[0]);
		}
    }
...
ATree.prototype.onElementDrop = function(dndManager, event, dragElement)
{
    ...
    //드롭하는 대상이 선택되어 있거나, 부모가 선택되어 있으면 이동시키지 않음
    if($dropItem.hasClass(this.selectStyle) || $(parent.pItem).children('label').hasClass(this.selectStyle)) return;
...
ATree.prototype.changeIcon = function(item, icon)
{
	this._changeIconByUrl(item, icon);
};

ATree.prototype._changeIconByCss = function(item, icon)
{
	var $icon = item.$icon;
	...
	$icon.css(
	{
		...
		'background-position': this._getBgPos(icon),//(-16 * icon) + 'px 0px',
...
ATree.prototype._changeIconByUrl = function(item, icon)
{
	var $icon = item.$icon;
	$icon.css(
	{
...
ATree.prototype._getBgPos = function(icon)
{
	var bgPos = [];
