...
AListView.prototype.createItems = function(url, dataArray, posItem, isPrepend, asyncCallback)
{
    ...
    for(var i=0; i<dataLen; i++)
    {
		...
		if(asyncCallback) 
		{
			if(i==0)
			{
				AView.createView(item, url, this, null, !this.option.isUpdatePosition, null, function(_aview)
				{
					...
					//비동기 로드가 완료 됐으므로 1부터 시작
					for(var j=1; j<dataLen; j++)
					{
						//_aview = AView.createView(newItems[j], url, thisObj, null, !thisObj.option.isUpdatePosition, null, null, AView.lastLoadedHtml);
						_aview = AView.createView(newItems[j], url, thisObj, null, !thisObj.option.isUpdatePosition, null, null, thisObj.lastLoadedHtml);
		...
		else 
		{
			...
			else aview = AView.createView(item, url, this, null, !this.option.isUpdatePosition, null, null, this.lastLoadedHtml);
