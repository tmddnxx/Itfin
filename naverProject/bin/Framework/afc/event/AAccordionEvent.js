...
AAccordionEvent.prototype._select = function(selItem)
{
	...
	$menu.bind(acc.option.showEvent, function(e)
	{
		...
		if(selItem.url && !selItem.contents.view)	//이미 로드되어져 있는 경우는 제외
		{
			...
			if(selItem.asyncCallback) 
			{
				AView.createView(selItem.contents, selItem.url, acc, null, null, null, function(_aview)
				{
					acc._afterCreated(_aview, selItem.data);

					if(typeof(selItem.asyncCallback)=='function') 
					{
						selItem.asyncCallback(_aview, selItem);
					}
				});
			}
			else 
			{
				var aview = AView.createView(selItem.contents, selItem.url, acc);
